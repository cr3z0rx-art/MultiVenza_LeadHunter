# Implementación: Base de Datos Simulada de Teléfonos

## Descripción
Este documento detalla la implementación del simulador de base de datos de teléfonos para el módulo de Skip Tracing. El simulador generará datos realistas de teléfonos para los propietarios de Georgia, tanto personas como empresas.

## Estructura del Archivo `phone_database_simulator.py`
...
1. **Clasificación de Propietarios**
   - Determina automáticamente si un nombre corresponde a una empresa o persona
   - Utiliza indicadores como "LLC", "Inc", etc. para empresas
   - Analiza el formato del nombre para personas (Nombre Apellido)

2. **Generación de Teléfonos**
   - Crea números de teléfono con códigos de área válidos de Georgia
   - Asigna tipos diferentes según sea empresa o persona
   - Simula diferentes fuentes de datos para cada teléfono

3. **Simulación de Éxito/Fracaso**
   - Permite configurar una tasa de éxito para simular casos donde no se encuentra teléfono
   - Genera entradas vacías para casos de fracaso

4. **Persistencia**
   - Guarda la base de datos en formato JSON
   - Permite cargar datos previamente generados

## Uso del Simulador

El simulador se puede utilizar de dos formas:

1. **Como script independiente**
   ```bash
   python scripts/phone_database_simulator.py
   ```
   Esto generará una base de datos con propietarios de ejemplo.
...
2. **Como módulo importado**
   ...
   # Crear simulador
   simulator = PhoneDatabaseSimulator()
   
   # Generar base de datos para propietarios específicos
   simulator.generate_database(["Atlanta Development LLC", "Robert Johnson"])
   
   # Buscar un teléfono
   phone, phone_type, source = simulator.lookup_phone("Atlanta Development LLC")
   ```

## Estructura de Datos

La base de datos generada tendrá el siguiente formato:
...
1. Implementar el script `phone_database_simulator.py` según esta especificación
2. Probar con la lista de propietarios de ejemplo
3. Integrar con el extractor de nombres reales de Georgia
4. Conectar con el módulo principal de Skip Tracing